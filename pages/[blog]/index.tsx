...
const Blog = ({ postData }: { postData: Posts }) => {
  ...
  return (
    <Layout TOC={tableOfContents}>
          ...
          <div className={Style["content-header"]}>
            <div className={Style["post-info"]}>
              ...
              <div className={Style["post-description"]}>
                <Link href="/creator/">
                  <a className="author">{author.fields.author}</a>
                </Link>
                <span className="datetime">{date}</span>
                {/* <span className="timeToRead">
                  {contentPost.childMarkdownRemark.timeToRead} min read
                </span> */}
              </div>
              {/* {sourceCode && (
                <a
      ...
      {/* <LikeCount postId={id} title={title} /> */}
      {/* </Sidebar> */}
    </Layout>
  );
};
...
export const getStaticPaths: GetStaticPaths = async () => {
  const entries = await fetchEntries("posts", 1000);
  console.log(entries);
